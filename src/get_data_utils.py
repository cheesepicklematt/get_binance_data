...
class getData:
            ...
            try:
                if len(self.time_str) == 1:
                    data = self.client.get_historical_klines(
                        asset, self.kline_interval, self.time_str[0]
                    )
                else:
                    data = self.client.get_historical_klines(
                        asset, self.kline_interval, self.time_str[0], self.time_str[1]
                    )
                ...
                data.columns = [
                    x + "_" + asset if x.find("time") == -1 else x for x in data.columns
                ]
    ...
    def merge_data(self, prices_list=None):
        ...
        for i in range(num_assets):
            final_data = pd.merge(
                final_data, prices_list[i], on="Open time", how="left"
            )
        return final_data
    ...
    def extract_data(self):
        ...
        for symbol_list in symbol_splits:
            thread = threading.Thread(
                target=self.get_data_thread,
                kwargs={"symbol_list": symbol_list, "data_queue": result_queue},
            )
        ...
        export_data["Open time formatted"] = self.convert_timestamps_to_ymd_hms(
            export_data["Open time"]
        )
...
class xHourVolume:
    ...
    def run(self):
        ...
        symbol_df = symbol_df.sort_values(by="btc_volume", ascending=False).head(
            self.top_x
        )
...
class tradeData:
    ...
    def _clean_data(self, symbol):
        self.trade_df["time_formatted"] = self.convert_timestamps_to_ymd_hms(
            self.trade_df["Timestamp_" + symbol]
        )
    ...
    def extract_raw_data(self):
        ...
        for symbol in self.symbol_list:
            agg_trades = self.client.aggregate_trade_iter(
                symbol=symbol, start_str=self.time_str
            )
            ...
            self.trade_df = pd.DataFrame(
                trade_data, columns=[x + "_" + symbol for x in self.dict_names.values()]
            )
...
def get_price_qty_round_num(symbol_list):
    ...
    for symbol in symbol_list:
        ...
        price_round_num = (
            len(price_round_num.split(".")[1])
            if price_round_num.split(".")[1] != "0"
            else 0
        )

        min_trade = symbol_info[1]["minQty"].rstrip("0")
        qty_round_num = (
            len(min_trade.split(".")[1]) if min_trade.split(".")[1] != "0" else 0
        )
    ...
    return pd.DataFrame(
        round_num_details, columns=["symbol", "price_round_num", "qty_round_num"]
    )
